...
class MakeTestCase:
    def __init__(self, test_case_dir, case_data):
        self.test_case_dir = test_case_dir
        self.case_data = case_data
        self.__getAttributes()
        # self.case_name=case_data['case_name']
        self.file_name = self.test_case_dir + r"/" + self.case_name + ".py"
        self.__create_test_case()

    # 生成用例
    def __create_test_case(self):
        try:
            with open(self.file_name, 'w', encoding='utf-8') as self.test_case:
                message = self.__write()
                self.test_case.write(message)
    ...
    def __write(self):
        try:
            template_dir = os.path.abspath(os.path.join(self.test_case_dir, "../../template"))
            with open(template_dir, 'r', encoding='utf-8') as self.template:
                template_message = self.template.read()
                # regex=r"\${.*?}"
                # 替换case_name
                regex = r"\${case_name}"
                message = self.__replaceVariable(regex, self.case_name, template_message)
                # 替换case_desc
                regex = r"\${case_desc}"
    ...
    # 替换变量
    def __replace_Variable(self, regex, variable, message, count=0):
        try:
    ...
    def __replace_Step(self, message):
        # 根据step_list_data的长度创建几个step
...
'''
            message += stepmessage
        # 替换变量
        regex_list = [r"\${step_name}", r"\${step_name}", r"\${step_desc}", r"\${params}", r"\${headers}", r"\${method}",
                      r"\${way}", r"\${assert_response}", r"\${sqlDatalist}", r"\${api_dependency}"]
        for i in range(self.steplen):
            j = 0
            # 替换步骤名,替换params和headers
            step_name = self.case_data['step_list_data'][i]['step_name']
            message = self.__replace_Variable(regex_list[j], step_name, message, 1)
            # 替换步骤名,替换变量
            j += 1
            message = self.__replace_Variable(regex_list[j], step_name, message, 1)
            ...
            message = self.__replace_Variable(regex_list[j], step_desc, message, 1)

            # 替换params
            j += 1
            params = self.case_data['step_list_data'][i]['params']
            message = self.__replace_Variable(regex_list[j], params, message, 1)
            # 替换headers
            j += 1
            headers = self.case_data['step_list_data'][i]['headers']
            message = self.__replace_Variable(regex_list[j], headers, message, 1)
            # 替换method
            j += 1
            method = self.case_data['step_list_data'][i]['method']
            message = self.__replace_Variable(regex_list[j], method, message, 1)
            # 替换way
            j += 1
            way = method
            message = self.__replace_Variable(regex_list[j], way, message, 1)
            # 替换断言
            j += 1
            if self.case_data['step_list_data'][i]['assert_response'] == "":
                message = self.__replace_Variable(regex_list[j], "{}", message, 1)
            else:
                message = self.__replace_Variable(regex_list[j], self.case_data['step_list_data'][i]['assert_response'],
                                                  message, 1)
            # 替换sql
            j += 1
            if len(self.case_data['step_list_data'][i]['sql_list_data']) == 0:
                ...
                message = self.__replace_Variable(regex_list[j], str(sqlDatalist), message, 1)
            # 替换接口依赖
            j += 1
            if self.case_data['step_list_data'][i]['api_dependency'] == "":
                message = self.__replace_Variable(regex_list[j], "{}", message, 1)
            else:
                message = self.__replace_Variable(regex_list[j],
    ...
    def __get_Attributes(self):
        ...
        self.step_plan = len(self.case_data['step_list_data'])
        # 得到每个脚本的sql数
        self.step_Sql_len = []
        for i in range(self.steplen):
            self.step_Sql_len.append(len(self.case_data['step_list_data'][i]['sql_list_data']))
